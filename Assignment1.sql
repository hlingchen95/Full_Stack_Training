...
SELECT p.Name, p.Color
FROM Production.Product p
WHERE p.Name LIKE '%Crankarm' OR p.Name LIKE 'Chainring%' AND p.Color IN ('Black','Silver')
...
FROM Production.Product p
WHERE p.Name LIKE 'Seat%' OR p.Name IN ('Short-Sleeve Classic Jersey, L','Short-Sleeve Classic Jersey, M')
ORDER BY p.Name
